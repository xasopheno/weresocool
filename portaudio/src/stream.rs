    ...
    I: Sample + 'static,
{
    ...
    unsafe fn new_callback_args(
        ...
        let time = InputCallbackTimeInfo {
        ...
        let buffer: &[I] = {
            ...
            std::slice::from_raw_parts(buffer_ptr, buffer_len)
        };
        InputCallbackArgs {
            buffer,
    ...
    O: Sample + 'static,
{
    ...
    unsafe fn new_callback_args(
        ...
        let buffer: &mut [O] = {
            ...
            std::slice::from_raw_parts_mut(buffer_ptr, buffer_len)
        };
        OutputCallbackArgs {
            buffer,
            frames: frame_count as usize,
    ...
    O: Sample + 'static,
{
    ...
    unsafe fn new_callback_args(
        ...
        let flags = CallbackFlags::from_bits(flags).unwrap_or_else(CallbackFlags::empty);
        let time = DuplexCallbackTimeInfo {
            current: (*time_info).currentTime,
...
fn open_blocking_stream(
    ...
    let in_c_params = in_params.map(|p| p);
    let out_c_params = out_params.map(|p| p);
