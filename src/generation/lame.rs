...
impl Lame {
    ...
    pub fn set_kilobitrate(&mut self, quality: i32) -> Result<(), Error> {
        handle_simple_error(unsafe {
            crate::generation::ffi::lame_set_brate(self.ptr, quality as c_int)
        })
    }
    ...
    pub fn init_params(&mut self) -> Result<(), Error> {
        handle_simple_error(unsafe { crate::generation::ffi::lame_init_params(self.ptr) })
    }
    ...
    pub fn encode(
        ...
        let retn = unsafe {
            crate::generation::ffi::lame_encode_buffer(
                self.ptr,
                pcm_left.as_ptr(),
                pcm_right.as_ptr(),
    ...
    pub fn encode_f32(
        ...
        let retn = unsafe {
            crate::generation::ffi::lame_encode_buffer_ieee_double(
                self.ptr,
                pcm_left.as_ptr(),
                pcm_right.as_ptr(),
                int_size(pcm_left.len()),
...
impl Drop for Lame {
    fn drop(&mut self) {
        unsafe { crate::generation::ffi::lame_close(self.ptr) };
    }
}
