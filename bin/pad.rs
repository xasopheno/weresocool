...
fn normalize_vec_string(vec_string: Vec<String>) {
    let mut composition = String::new();
    for line in vec_string {
        let l = line;
...
fn run() -> Result<(), Error> {
    ...
    let from_filename = normalize_vec_string(filename_to_vec_string(filename));
    let from_language = normalize_vec_string(language_to_vec_string(language.as_str()));
