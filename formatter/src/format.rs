...
use weresocool_parser::{Comment, Init, ParsedComposition};
...
pub fn format_composition_with_source(parsed: &ParsedComposition, original_source: &str, span_source: &str, config: &FormatConfig) -> String {
    format_composition_with_source_and_comments(parsed, original_source, span_source, &[], config)
}

/// Format with access to original source and comments
pub fn format_composition_with_source_and_comments(
    ...
    // Re-insert comments at appropriate positions
    if !comments.is_empty() {
        output = insert_comments(&output, original_source, comments);
    }
...
/// Insert comments back into formatted output.
/// Comments are mapped to their original line positions.
fn insert_comments(formatted: &str, original_source: &str, comments: &[Comment]) -> String {
    if comments.is_empty() {
        return formatted.to_string();
    }

    // Build a map of original line number -> what the line starts with (for matching)
    let original_lines: Vec<&str> = original_source.lines().collect();
    let formatted_lines: Vec<&str> = formatted.lines().collect();

    // For each comment, find where to insert it in the formatted output
    // Strategy: look at what non-comment line follows the comment in the original,
    // then find that line (or similar) in the formatted output
    let mut result_lines: Vec<String> = Vec::new();
    let mut formatted_idx = 0;
    let mut comment_idx = 0;

    // Sort comments by line number
    let mut sorted_comments: Vec<&Comment> = comments.iter().collect();
    sorted_comments.sort_by_key(|c| c.line);

    while formatted_idx < formatted_lines.len() || comment_idx < sorted_comments.len() {
        // Check if we have a comment that should be inserted here
        if comment_idx < sorted_comments.len() {
            let comment = sorted_comments[comment_idx];

            // Find the next non-comment, non-blank line after this comment in the original
            let mut next_content_line = comment.line + 1;
            while next_content_line < original_lines.len() {
                let line = original_lines[next_content_line].trim();
            ...
            // Get the content that should follow the comment
            let following_content = if next_content_line < original_lines.len() {
                // Get first significant token from the line
                extract_first_token(original_lines[next_content_line])
            } else {
                None
            };

            // Check if current formatted line matches what should follow the comment
            let should_insert = if let Some(token) = following_content {
                formatted_idx < formatted_lines.len() &&
                    extract_first_token(formatted_lines[formatted_idx]) == Some(token)
            } else {
                // Comment is at the end of file
                formatted_idx >= formatted_lines.len()
            };

            if should_insert {
                // Get indentation - if the next line is a closing bracket, use previous line's indent
                let indent = if formatted_idx < formatted_lines.len() {
                    let next_line = formatted_lines[formatted_idx].trim();
                    if next_line.starts_with(']') || next_line.starts_with('}') {
                        // Comment before closing bracket - use indent of previous content line
                        if !result_lines.is_empty() {
                            get_leading_whitespace(&result_lines[result_lines.len() - 1])
                        } else {
                            get_leading_whitespace(formatted_lines[formatted_idx])
                        }
                    } else {
                        get_leading_whitespace(formatted_lines[formatted_idx])
                    }
                ...
                // Re-indent the comment to match
                let comment_text = comment.text.trim_start();
                result_lines.push(format!("{}{}", indent, comment_text));
                comment_idx += 1;
                continue;
            }
        }

        // Add the next formatted line
        if formatted_idx < formatted_lines.len() {
            result_lines.push(formatted_lines[formatted_idx].to_string());
            formatted_idx += 1;
        } else {
            break;
        }
    ...
    while comment_idx < sorted_comments.len() {
        result_lines.push(sorted_comments[comment_idx].text.clone());
        comment_idx += 1;
    }
...
/// Extract the first significant token from a line for matching purposes
fn extract_first_token(line: &str) -> Option<&str> {
    let trimmed = line.trim();
    if trimmed.is_empty() {
        return None;
    }

    // Find the first word/identifier
    let end = trimmed.find(|c: char| c.is_whitespace() || c == '=' || c == '(' || c == '{')
        .unwrap_or(trimmed.len());

    let token = &trimmed[..end];
    if token.is_empty() {
        // Handle special cases like "{ f:" for init block
